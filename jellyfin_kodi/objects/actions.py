...
class Actions(object):
    ...
    def play(self, item, db_id=None, transcode=False, playlist=False):
        ...
        # When we have multiple items (e.g., cinema mode intros + movie),
        # we need to use the playlist approach instead of setResolvedUrl
        # which can only handle a single file.
        if len(self.stack) > 1:
            self._play_stack()
        else:
    ...
    def _play_stack(self):
        ...
        # Set a flag so the player knows to pause briefly after the preroll starts
        # This allows the TV to complete refresh rate/HDMI sync before the intro plays
        window("jellyfin.cinema_mode_pause", value="true")
